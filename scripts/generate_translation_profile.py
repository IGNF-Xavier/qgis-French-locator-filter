...
# -- Variables
src_path = Path("french_locator_filter")
i18n_path: Path = src_path.joinpath("resources/i18n")
output_file: Path = i18n_path.joinpath("plugin_translation.pro")
...
ui_files: list[Path] = [f for f in sorted(list(Path(src_path).rglob("*.ui")))]
ts_files: list[Path] = [f for f in sorted(list(Path(src_path).rglob("*.ts")))]
